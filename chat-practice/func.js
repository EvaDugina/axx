...
function auth($db,$login,$pass) {
	...
	//$result = mysqli_query($db,"SELECT * FROM userlist WHERE login='$login' AND password='$pass'");//заменить на чтение из txt
	$result = getXmlHttp();
    if($result) {
		if($result == 1) {//Проверяем, одно ли совпадение
			$user = $result; //Получаем данные пользователя и заносим их в сессию
...
function load($db)0 { //поменять для txt файла
	$echo = "";
...
function send($db,$message) { //тоже  через txt 
