...
?>
		<html>
		<header><title>Accelerator API</title></header>
		<body>
...
<?php	
	...
	if (!$flag_autotest)
		...
		$result = pg_query($dbconnect,  "SELECT f.* from ax_file f INNER JOIN ax_task_file ON ax_task_file.file_id = f.id inner join ax_assignment a on ax_task_file.task_id = a.task_id where f.type = 2 and a.id = ".$assignment_id);
