...
files_regex_to_exclude = [
    'auth.php', 'auth_ssh_*.php', 'update.php', 'update_action.php', 'env.php', 'dbparams.php'
]
    ...
    for file_regex in files_regex_to_exclude:
        for stage_file in staged_files:
            if re.search(file_regex, stage_file):
                print(f"{red_color}Error! File {stage_file} can't be in commit.")
                return False
...
if __name__ == "__main__":
    if check_files():
        print(f"{green_color}All files are approved.")
        sys.exit(0)
    else:
        print(f"{red_color}Failed! Commit is rejected!")
        sys.exit(1)