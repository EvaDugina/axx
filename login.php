...
<body style="overflow-x: hidden;">
      ...
      <div class="row gy-5">
        ...
        <div class="col-12 col-xl-4 pt-4">
          <div class="pt-1 px-3" style="border: 1px solid #dee2e6; border-radius: 5px;">
            ...
            <form id="form-auth" class="text-nowrap form-horizontal" method="post" action="auth.php" onsubmit="validateForm()">
              <input type="hidden" name="action" value="login" />

              <div class="form-outline my-3">
                <input type="text" id="input-login" name="login" class="form-control" onchange="hideAuthorizationError()" />
                <label class="form-label" for="login">Логин</label>
              </div>

              <div class="form-outline my-3">
                <input type="password" id="input-password" name="password" class="form-control" onchange="hideAuthorizationError()" />
                <label class="form-label" for="pass">Пароль</label>
              </div>

              <strong>
                <p id="error-authorization" class="error <?= ($errorAuth) ? "" : "d-none" ?>">Ошибка: Неверный Логин или Пароль</p>
              </strong>

              <strong>
                <p id="error-field-filled" class="error d-none">Ошибка: Незаполненные поля!</p>
              </strong>
  ...
  <script type="text/javascript">
    function hideAuthorizationError() {
      document.getElementById("error-authorization").classList.add("d-none");
    }
